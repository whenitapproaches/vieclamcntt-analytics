...
function searchForHiringPhrase(paragraph) {
  let lines = contentToLines(paragraph)
...
function searchForFirstLanguage(paragraph) {
  ...
  let searchedLanguages = [...joinedLanguages, ...searchLanguagesWithDash].map(
    (language) => {
      let searchLanguage =
        language.length < 3 ? language : language.toLowerCase()
...
function checkTypeOfList(content) {
  ...
  // if(orderedListsByListStyleFirst.length === 0 && orderedListsByListStyleSecond.length === 0) {
  //   let jobs = checkUnorderedListLines(possibleListLines)
  // }
  ...
  if (arrayOfJobs.length !== 0) {
    ...
    if (notFoundJobs) {
      ...
      if (undefinedLength / arrayOfJobsLength > 0.65 && undefinedLength > 5) {
...
function checkOrderedList(lines) {
  ...
  linesWithNumberListStyle.forEach((line) => {
    ...
    if (isNaN(lineCurrentValue)) return
    console.log(lineCurrentValue, linePreviousValue)
    if (lineCurrentValue === linePreviousValue + 1) {
